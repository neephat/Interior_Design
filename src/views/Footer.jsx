    ...
    <div className="max-w-[1490px] mx-auto px-10 py-[80px]">
        ...
        <div className="w-full xl:w-auto flex flex-col items-center xl:items-start">
          ...
          <div className="relative w-full xs:w-[380px] xl:w-[320px] mt-11">
            <input 
              type="text" 
              placeholder="Email" 
              className="rounded-full w-full pl-6 pr-[110px] xs:pr-[140px] py-4 bg-primary outline-none text-white text-base xs:text-lg placeholder-white" 
              />
            <button className="absolute top-2/4 -translate-y-2/4 right-3 bg-[linear-gradient(90deg,#2AF598_0%,#009EFD_100%)] py-1 px-2 xs:px-6 xs:py-2.5 rounded-full cursor-pointer text-white">
              Subscribe
            </button>
          </div>
        </div>
              ...
              <div key={index} className="text-white max-w-screen md:max-w-[340px] flex-1 md:flex-auto">
                ...
                <ul className="flex flex-col gap-4">
                  {item.links.map((link, i) => (
                    <div key={`list-item-${i}`} className="w-full flex mx-auto justify-center xl:justify-start">
                      <li 
                        className={`font-medium text-sm md:text-base ${index !== 1 && "cursor-pointer"} text-center xl:text-start w-fit`}
                      >
                        {link}
                      </li>
                    </div>
      ...
      <div className="w-full h-[1px] bg-white my-12" />
        <h6 className="text-white text-center text-base lg:text-lg">
          &copy; Neephat Benazir - All rights reserved
        </h6>
